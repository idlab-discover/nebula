...
impl CliCommand for InstrumentCommand {
	...
	async fn handle(&self) -> Result<()> {
		...
		let input_path = self.input.canonicalize().with_context(|| {
			format!("failed to resolve input path: {}", self.input.display())
		})?;
		...
		let input_bytes = std::fs::read(&input_path).with_context(|| {
			format!("failed to read input component: {}", input_path.display())
		})?;

		let graph = parse_component(&input_bytes).with_context(|| {
			format!("failed to parse component: {}", input_path.display())
		})?;
		...
		let (splits_path, shim_comps) =
			split_out_composition(&input_path, &Some(splits_dir_str))?;
		...
		let composed = compose_wac_to_component(
			&wac_out.wac,
			wac_out.wac_deps,
			&working_dir,
		)?;
...
fn build_splice_rules(
	...
	if !tracing_path.exists() {
		bail!("missing tracing proxy component at {}", tracing_path.display());
	}

	let tracing_path =
		tracing_path.into_os_string().into_string().map_err(|_| {
			anyhow::anyhow!("tracing proxy path contains non-UTF-8 bytes")
		})?;

	// todo(ewout): dynamically build rules based on all discovered exported
	// interfaces and requested instrumentation(s).
	let rules = vec![
		SpliceRule::Before {
			interface:      "nebula:service/orders".to_string(),
			provider_name:  Some("service".to_string()),
			provider_alias: Some("srv-a".to_string()),
			inject:         vec![Injection::from_path(
				"tracing",
				tracing_path.clone(),
			)],
		},
		SpliceRule::Before {
			interface:      "nebula:service/quotes".to_string(),
			provider_name:  Some("service".to_string()),
			provider_alias: Some("srv-b".to_string()),
			inject:         vec![Injection::from_path(
				"tracing",
				tracing_path.clone(),
			)],
