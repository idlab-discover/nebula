...
use crate::bindings::wasi::otel::types::{
...
impl Span {
	...
	pub fn start(&self) { tracing::on_start(&self.context); }
	...
	pub fn set_kind(&mut self, kind: SpanKind) { self.kind = kind; }
	...
	pub fn set_status(&mut self, status: Status) { self.status = status; }
	...
	pub fn add_link(
		...
		self.links.push(Link { span_context: context, attributes });
	}
}

/// A tracer is responsible for creating spans and managing the lifecycle of a
/// trace. It provides methods to start and end spans, as well as to set
/// attributes and status on spans.
pub struct Tracer {
