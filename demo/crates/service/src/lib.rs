...
impl OrdersGuest for Service {
	...
	fn create_order(order: Order) -> Quote {
		...
		let tax = TRACER.start_span("calculate-tax", |_| {
		...
		let total =
			TRACER.start_span("calculate-total", |_| subtotal * (1.0 + tax));
		...
		Quote { order_id: order.order_id, subtotal, tax, total, currency }
	}
